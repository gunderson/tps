...
var Visualizer = function(options) {
	...
	function computeSpiralPosition() {
		...
		p.position.y = 200 * Math.pow(1 - indexScale, 3);
	...
	function computeGridPosition(cols, rows) {
		...
		p.homePosition.z = ((p.gridPosition.y / rows) * gridHeight) - (gridHeight * 0.5);

		var homeRatioX = 2 * p.homePosition.x / gridWidth;
		var homeRatioY = 2 * p.homePosition.z / gridHeight;

		p.homePosition.y = 25 * (Math.sin(homeRatioX * 0.5 * Math.TAU) + Math.sin(homeRatioY * 0.25 * Math.TAU)) + 0 * homeRatioX + 0 * homeRatioY;
